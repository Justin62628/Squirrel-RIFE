...
class IFNet(nn.Module):
    ...
    def get_auto_scale(self, x):
        ...
        flow, mask = block[0](torch.cat((img0[:, :3], img1[:, :3], timestep), 1), None, scale=8)
        warped_img0 = warp(img0, flow[:, :2])
        warped_img1 = warp(img1, flow[:, 2:4])
        f0, m0 = block[1](torch.cat((warped_img0[:, :3], warped_img1[:, :3], timestep, mask), 1), flow,
                          scale=4)
        if f0[:, :2].abs().max() > threshold and f0[:, 2:4].abs().max() > threshold:
            scale = 0.5
            flow, mask = block[0](torch.cat((img0[:, :3], img1[:, :3], timestep), 1), None, scale=16)
            warped_img0 = warp(img0, flow[:, :2])
            warped_img1 = warp(img1, flow[:, 2:4])
            f0, m0 = block[1](torch.cat((warped_img0[:, :3], warped_img1[:, :3], timestep, mask), 1), flow,
                              scale=8)
    ...
    def forward(self, x, timestep=0.5, scale_list=[8, 4, 2, 1], training=False, fastmode=True, ensemble=False, ada=True):
        # if training == False:
        channel = x.shape[1] // 2
        img0 = x[:, :channel]
        img1 = x[:, channel:]
