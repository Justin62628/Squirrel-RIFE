...
class IFNet_m(nn.Module):
    ...
    def forward(self, x, scale=[4, 2, 1], timestep=0.5, ensemble=False):
        ...
        for i in range(3):
            if flow != None:
                flow_d0, mask_d0 = stu[i](torch.cat((img0, img1, timestep, warped_img0, warped_img1, mask), 1), flow, scale=scale[i])
                if ensemble:
                    flow_d1, mask_d1 = stu[i](torch.cat((img1, img0, 1-timestep, warped_img1, warped_img0, -mask), 1), torch.cat((flow[:, 2:4], flow[:, :2]), 1), scale=scale[i])
                    flow_d0 = (flow_d0 + torch.cat((flow_d1[:, 2:4], flow_d1[:, :2]), 1)) / 2
                ...
                if ensemble:
                    flow1, mask1 = stu[i](torch.cat((img1, img0, 1-timestep), 1), None, scale=scale[i])
                    flow = (flow + torch.cat((flow1[:, 2:4], flow1[:, :2]), 1)) / 2
                    mask = (mask + (-mask1)) / 2
