...
class IFNet(nn.Module):
    ...
    def forward(self, x, scale=[4,2,1], ensemble=False):
        ...
        for i in range(3):
            if flow != None:
                flow_d0, mask_d0 = stu[i](torch.cat((img0, img1, warped_img0, warped_img1, mask), 1), flow, scale=scale[i])
                if ensemble:
                    flow_d1, mask_d1 = stu[i](torch.cat((img1, img0, warped_img1, warped_img0, -mask), 1), torch.cat((flow[:, 2:4], flow[:, :2]), 1), scale=scale[i])
                ...
                if ensemble:
                    flow1, mask1 = stu[i](torch.cat((img1, img0), 1), None, scale=scale[i])
